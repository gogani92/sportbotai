...
export default function TacticalAnalysisSection({ tacticalAnalysis }: TacticalAnalysisSectionProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative bg-gradient-to-br from-bg-hover to-bg-card rounded-xl p-4 border border-divider overflow-hidden">
        <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-primary-900 to-accent-cyan"></div>
        <div className="pl-3">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-base">🎮</span>
            <h4 className="text-xs font-semibold text-text-muted uppercase tracking-wider">Playing Styles</h4>
          </div>
      ...
      <div className="bg-bg-card rounded-xl p-4 border border-divider shadow-sm">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h4 className="text-sm font-semibold text-white">Match Narrative</h4>
        </div>
        <p className="text-sm text-text-secondary leading-relaxed">
          {matchNarrative}
        </p>
      ...
      {keyMatchFactors && keyMatchFactors.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] text-text-muted">{keyMatchFactors.length} factors</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {keyMatchFactors.map((factor, index) => (
              <div
                key={index}
                className="group flex items-start gap-3 p-3 bg-bg-hover rounded-lg border border-divider hover:border-primary-900/20 hover:bg-bg-hover transition-all duration-200"
              >
